	...
	public function add_vehicle_post()
    {
		...
		if (!empty($user)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (isset($_FILES['vehicle_owner_id_proof_photo']['size']) > 0) {
					$params['vehicle_owner_id_proof_photo'] = $this->common->upload_image ( 'vehicle_owner_id_proof_photo', UPLOAD_VEHICLE_PATH );
				}

				// Rename Vehicle Owner Photo
				if (isset($_FILES['vehicle_owners_photo']) && strlen($params['vehicle_owners_photo']) > 0) {
					$params['vehicle_owners_photo'] = $this->common->upload_image ( 'vehicle_owners_photo', UPLOAD_VEHICLE_PATH );
				}

				// Rename RC Book Photo
				if (isset($params['rc_book_photo']) && strlen($params['rc_book_photo']) > 0) {
					$params['rc_book_photo'] = $this->common->upload_image ( 'rc_book_photo', UPLOAD_VEHICLE_PATH );
				}

				// Rename Vehicle Speed Governer Photo
				if (isset($params['veh_speed_governer_photo']) && strlen($params['veh_speed_governer_photo']) > 0) {
					$params['veh_speed_governer_photo'] = $this->common->upload_image ( 'veh_speed_governer_photo', UPLOAD_VEHICLE_PATH );
				}

				// Rename Vehicle Photo
				if (isset($params['veh_photo']) && strlen($params['veh_photo']) > 0) {
					$params['veh_photo'] = $this->common->upload_image ( 'veh_photo', UPLOAD_VEHICLE_PATH );
				}
	...
	*/
	public function list_get()
    {
		...
		if (!empty($user)) {
			...
			$veh_id       = (int)$this->input->get('veh_id');
			$vehicle_list = $this->vehiclemodel->vehicle_list($user, $search_param, $veh_id);
			...
			$this->returnResponse['status'] = true;
			$this->returnResponse['msg']    = "Vehicle sent successfully";
